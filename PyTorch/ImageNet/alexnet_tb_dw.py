...
class AlexNetTbDw(nn.Module):
    def __init__(self, num_classes=1000, threshold=0):
        ...
        self.features = nn.Sequential(
            ...
            ConvBlock(96, 256, kernel_size=5, stride=1, padding=2, groups=1, threshold=threshold),
            nn.MaxPool2d(kernel_size=3, stride=2),
            ConvBlock(256, 384, kernel_size=3, stride=1, padding=1, threshold=threshold),
            ConvBlock(384, 384, kernel_size=3, stride=1, padding=1, groups=1, threshold=threshold),
            ConvBlock(384, 256, kernel_size=3, stride=1, padding=1, groups=1, threshold=threshold),
            nn.MaxPool2d(kernel_size=3, stride=2), )

        self.classifier = nn.Sequential(ConvBlock(256 * 6 * 6, 4096, Linear=True, threshold=threshold),
            ConvBlock(4096, 4096, dropout=0.5, Linear=True, threshold=threshold),
