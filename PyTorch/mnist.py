...
def save_state(model, acc):
    ...
    torch.save(state, 'MNIST/' + args.arch + '.best.pth.tar')
...
def test(evaluate=False):
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)'.format(test_loss * args.batch_size, correct,
...
if __name__ == '__main__':
    ...
    for key, value in param_dict.items():
        params += [{'params': [value], 'lr': args.lr, 'weight_decay': args.weight_decay, 'key': key}]
