...
__global__ void _functionReduce(
    T* g_odata, unsigned int n, T initVal, UnaryFunction fcn,
    BinaryFunction fcn1) {
  T myVal = initVal;
...
inline void partialReduce(
    const int n, T** d_partialVals, T initVal, UnaryFunction const& fcn,
    BinaryFunction const& fcn1) {
  if (*d_partialVals == NULL)
    cudaMalloc(d_partialVals, (N_BLOCKS + 1) * sizeof(T));

  _functionReduce<T>
      <<<N_BLOCKS, N_THREADS>>>(*d_partialVals, n, initVal, fcn, fcn1);
}

template <typename T, typename UnaryFunction, typename BinaryFunction>
inline T functionReduce(
    const int n, T** d_partialVals, T initVal, UnaryFunction const& fcn,
    BinaryFunction const& fcn1) {
  ...
  CHECK(
      cudaMemcpy(
          h_partialVals, *d_partialVals, sizeof(T) * N_BLOCKS,
          cudaMemcpyDeviceToHost) == cudaSuccess)
      << "_functionReduce copy failed!";
