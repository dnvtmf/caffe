...
solver.train(base_lr=0.1, lr_policy='step', gamma=0.96, stepsize=320000,
...
out = Pool(out, name='resnet1_pool', method=Net.MaxPool, kernel_size=3,
           stride=2)
...
def neck_block(out_, num_output, first=False, stride=1):
    ...
    out_ = Conv(out_, name='conv1', conv_type=conv_type, num_output=num_output,
                ...
                weight_filler=filler_xavier, bias_term=False,
                optional_params=other_param)
    ...
    out_ = Conv(out_, name='conv3', conv_type=conv_type,
                num_output=num_output * 4, kernel_size=1, stride=1, pad=0,
                weight_filler=filler_xavier, bias_term=False,
                optional_params=other_param)
    # out_ = BN(out_, name='conv3_bn')
    if first:
        x = Conv(x, name='conv_shortcut', conv_type=conv_type,
                 num_output=num_output * 4, kernel_size=1, stride=stride,
                 pad=0, bias_term=False, weight_filler=filler_xavier,
                 optional_params=other_param)
...
out = Pool(out, name='avg_pool', method=1, global_pooling=True, stride=None,
           kernel_size=None)
out = FC(out, name='fc', num_output=1000, weight_filler=filler_xavier,
