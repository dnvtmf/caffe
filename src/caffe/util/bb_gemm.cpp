...
void block_gemm(
    const int index, const int M, const int N, const int K, const Btype *A,
...
void bb_gemm_cpu(
  ...
  CHECK(M % BLOCK_M == 0) << "M % BLOCK_M = " << M << " % " << BLOCK_M
                          << " != 0";
  CHECK(N % BLOCK_N == 0) << "N % BLOCK_N = " << N << " % " << BLOCK_N
                          << " != 0";
  CHECK(K % BLOCK_K == 0) << "K % BLOCK_K = " << K << " % " << BLOCK_K
                          << " != 0";
  const int num_kernel = M / BLOCK_M * N / BLOCK_N;
  for (int i = 0; i < num_kernel; ++i)
    block_gemm<Dtype>(i, M, N, K, A, A_scale, B, B_scale, C);
}

template void bb_gemm_cpu<float>(
    const int M, const int N, const int K, const Btype *A, const float *A_scale,
    const Btype *B, const float *B_scale, float *C);
template void bb_gemm_cpu<double>(
