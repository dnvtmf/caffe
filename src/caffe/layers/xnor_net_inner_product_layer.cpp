...
void XnorNetInnerProductLayer<Dtype>::Forward_cpu(
  ...
  caffe_cpu_gemm<Dtype>(
      ...
      sum_multiplier_.cpu_data(), weight_data, Dtype(0), w_bias_);
  caffe_scal<Dtype>(N_, -1. / K_, w_bias_);
  caffe_cpu_gemm<Dtype>(
      CblasNoTrans, CblasNoTrans, K_, N_, 1, Dtype(1),
      sum_multiplier_.cpu_data(), w_bias_, Dtype(1), weight_data);
  // clamp
  caffe_cpu_clip<Dtype>(K_ * N_, -1, 1, weight_data);
...
if (is_w_bin_) {
  caffe_cpu_binary_gradient<Dtype>(
      1, K_, N_, false, this->blobs_[0]->cpu_data(), w_scale_, w_bias_,
      weight_diff);
} else if (is_in_bin_) {
...
if (is_in_bin_) {
  caffe_cpu_binary_gradient<Dtype>(
      0, M_, K_, false, bottom[0]->cpu_data(), in_scale_, in_bias_,
      in_diff);
} else if (is_w_bin_) {
  caffe_cpu_ternary_gradient<Dtype>(
      0, M_, K_, false, bottom[0]->cpu_data(), in_scale_, in_bias_,
