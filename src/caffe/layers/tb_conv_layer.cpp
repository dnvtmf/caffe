...
void TBConvolutionLayer<Dtype>::forward_cpu_gemm(
  ...
  const Dtype *weights = (is_in_bin_ || is_w_bin_) ? weight_.cpu_data() :
  ...
  if (is_in_bin_) {
    ...
    caffe_cpu_ternary_approx<Dtype>(
      1, K_, N_, col_buff,
      in_.mutable_cpu_data(), in_s_.mutable_cpu_data(), in_s_.mutable_cpu_diff());
    col_buff = in_.cpu_data();
  }
...
void TBConvolutionLayer<Dtype>::backward_cpu_gemm(
  ...
  const Dtype *weight = (is_in_bin_ || is_w_bin_) ? weight_.cpu_data() :
                        this->blobs_[0]->cpu_data();
  const Dtype* col_buff = input;
  ...
  if (is_in_bin_) {
    ...
    caffe_cpu_ternary_approx<Dtype>(
      1, K_, N_, col_buff, in_.mutable_cpu_data(),
...
//  }
  if (is_w_bin_) {
    caffe_cpu_binary_approx<Dtype>(
      0, M_, K_, this->blobs_[0]->cpu_data(),
...
void TBConvolutionLayer<Dtype>::Backward_cpu(
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (this->param_propagate_down_[0] || propagate_down[i]) {
      for (int n = 0; n < this->num_; ++n) {
        backward_cpu_gemm(bottom_data + n * this->bottom_dim_,
  ...
  if (is_w_bin_) {
    caffe_cpu_binary_gradient<Dtype>(0, M_, K_, this->blobs_[0]->cpu_data(),
                                     weight_s_.cpu_data(), weight_diff);
  }
  else if (is_in_bin_) {
    caffe_cpu_ternary_gradient<Dtype>(
      0, M_, K_, this->blobs_[0]->cpu_data(),
      weight_s_.cpu_data(), weight_s_.cpu_diff(), weight_diff);
  }
}
